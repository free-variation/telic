...
# built into a shared library that telic dlopens via FFI: Accelerate on
...
LAPACKE_SHARED   = $(LAPACKE_DIR)/liblapacke_telic.so
...
all: telic $(LAPACKE_SHARED)

telic: $(SRCS) $(HDRS) $(TELIC_DEPS)
	$(CC) $(CFLAGS) $(TELIC_INCS) -o telic $(SRCS) $(TELIC_DEPS) $(LDLIBS) $(LDLIBS_CC)
...
src/c/lib_embed.h: $(FORTH_SRCS) Makefile
	scratch=$$(mktemp -d) && cat $(FORTH_SRCS) > $$scratch/lib.telic && (cd $$scratch && xxd -i lib.telic) > src/c/lib_embed.h && rm -rf $$scratch

src/c/logo_embed.h: telic-logo.txt
	xxd -i telic-logo.txt > src/c/logo_embed.h
...
acceptance-refs: telic
	@fail=0; for f in tests/acceptance/*.telic; do \
		./telic -b "$$f" | diff -q - "$${f%.telic}.expected" > /dev/null \
			|| { echo "FAIL $$f"; fail=1; }; \
	done; \
...
# its external deps (LAPACK via liblapacke_telic, xgboost via libxgboost).
