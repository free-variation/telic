...
const HelpEntry help_entries[] = {
	...
	{ "<", "( a b -- bool ) or ( m/arr x -- m )", "less-than; element-wise 1/0 mask on matrix operands (scalar broadcast) and on array operands (val_cmp per element, n×1)", "3 (float)", "matrix 1m(r×c)", "same; matrix O(r×c)", 4 },
	{ "<=", "( a b -- bool ) or ( m/arr x -- m )", "less-than-or-equal (≤); element-wise 1/0 mask on matrix operands (scalar broadcast) and on array operands (val_cmp per element, n×1)", "3 (float)", "matrix 1m(r×c)", "same; matrix O(r×c)", 4 },
	{ "=", "( a b -- bool )", "structural equality", "3 (float)", "none", "float O(1); string O(|s|); array/set O(n); frame O(n); matrix O(r×c)", 4 },
	{ ">", "( a b -- bool ) or ( m/arr x -- m )", "greater-than; element-wise 1/0 mask on matrix operands (scalar broadcast) and on array operands (val_cmp per element, n×1)", "3 (float)", "matrix 1m(r×c)", "same; matrix O(r×c)", 4 },
	{ ">=", "( a b -- bool ) or ( m/arr x -- m )", "greater-than-or-equal (≥); element-wise 1/0 mask on matrix operands (scalar broadcast) and on array operands (val_cmp per element, n×1)", "3 (float)", "matrix 1m(r×c)", "same; matrix O(r×c)", 4 },
	...
	{ "binary-dir", "( -- s )", "The directory holding the running water binary, symlinks resolved (realpath), so an installation's resources are reachable from any cwd; errors on the wasm build (no executable path)", "1", "1o", "O(|path|)", 31 },
	...
	{ "draw-node", "( node depth -- )", "statistics.h2o: print one fit-tree node and its subtree indented at depth; the recursion draw-tree drives from the root", "subtree", "strings", "O(nodes)", 18 },
	...
	{ "eq", "( a b -- bool ) or ( m/arr x -- m )", "equality; element-wise 1/0 mask on matrix and array operands (scalar broadcast; val_cmp per array element) — the mask-producing twin of =, which stays structural on collections. NaN elements equal nothing", "3 (float)", "matrix 1m(r×c)", "same; matrix O(r×c)", 4 },
	...
	{ "floats>matrix", "( ptr n -- m )", "Copy n 32-bit floats from foreign memory at ptr into a fresh n×1 double matrix — the read-back for a C call that returns a float const* result buffer (e.g. predictions); errors if n < 1", "n", "1m(n×1)", "O(n)", 34 },
	...
	{ "n-observations", "( x -- n )", "statistics.h2o: observation count of a fit input — an array's size, a matrix's row count", "8", "none", "O(1)", 18 },
	...
	{ "to", "( val -- )", "Assign to the named local (in a definition) or global. At interpreted top level — the REPL, a program file, a loaded file — it auto-creates an absent global. In a compiled body, a colon definition or a quotation alike, the global must already exist: to: unknown variable: <name>; declare it with variable. May trigger superword store-fusion while compiling.", NULL, NULL, NULL, 10 },
	...
	{ "where", "( m -- v )", "Flat row-major indices of the nonzero elements, as a k×1 index vector (1×k for a 1×n mask); composes with the </> masks and select-rows", "1 + n", "1m(k)", "O(n)", 18 },
	...
	{ "x-label", "( s -- )", "x-axis title, centered below the tick labels", NULL, NULL, NULL, 41 },
	...
	{ "y-label", "( s -- )", "y-axis title, rotated, centered beside the tick labels", NULL, NULL, NULL, 41 },
...
const int help_entry_count = 649;
...
const HelpExample help_examples[] = {
	...
	{ "draw-node", "{ :x [ 1 2 3 4 ] vector } [ 10 10 20 20 ] vector { } fit-tree 1 draw-node", "  x <= 2.5\n    predict 10  (n 2)\n  x > 2.5\n    predict 20  (n 2)" },
	...
	{ "n-observations", "[ 1 2 3 ] n-observations . [ 1 2 3 ] vector n-observations . cr", "3 3" },
	...
	{ "pfit-tree", "{ :x [ 1 2 3 4 ] vector } [ 10 10 20 20 ] vector { } pfit-tree :threshold @ . cr", "2.5" },
	...
	{ "timed", "[: 1000 iota [: 1+ :] map drop :] timed", "1.00001e-05" },
	...
	{ "yield", ": nums 1 yield 2 yield ; ' nums 2 gen-take . cr", "[ 1 2 ]" },
	{ "{", "{ :a 1 :b 2 } frame>array . cr", "[ :a 1 :b 2 ]" },
	{ "|", ": hyp | a b | to b to a a a * b b * + sqrt ; 3 4 hyp . cr", "5" },
	{ "|>", ": diff |> a b | a b - ; 10 3 diff . cr", "7" },
	{ "}", "{ :x 9 } :x @ . cr", "9" },
	{ "~", "[ 1 2 ] [ 1 2 ] ~ . cr", "[ 1 2 ]" },
};

const int help_example_count = 650;
