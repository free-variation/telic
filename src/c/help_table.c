...
const HelpEntry help_entries[] = {
	...
	{ "abline", "( slope intercept -- )", "The line y = slope·x + intercept, clipped to the domain at render; aes :line-stroke :line-width", NULL, NULL, NULL, 47 },
	...
	{ "barchart-plot", "( heights labels -- svg )", "Complete bar chart (border and y-ticks, no x-ticks), rendered", NULL, NULL, NULL, 47 },
	...
	{ "boxplot", "( data -- )", "One Tukey boxplot centered in the figure (whiskers to 1.5·IQR, outliers as circles); pins the domain; aes :box-stroke", NULL, NULL, NULL, 47 },
	{ "boxplot-plot", "( data -- svg )", "Complete single boxplot, rendered", NULL, NULL, NULL, 47 },
	{ "boxplots", "( arrays labels -- )", "Side-by-side boxplots on a shared y axis, one per vector; labels a parallel array of category strings; pins the domain", NULL, NULL, NULL, 47 },
	{ "boxplots-plot", "( arrays labels -- svg )", "Complete side-by-side boxplots, rendered", NULL, NULL, NULL, 47 },
	...
	{ "count-barchart", "( values -- )", "Frequency bars — one per distinct value, height its number of occurrences, most frequent first; labels the values rendered as strings; aes :bar-fill :bar-stroke", NULL, NULL, NULL, 47 },
	{ "count-barchart-plot", "( values -- svg )", "Complete frequency bar chart from raw values, rendered", NULL, NULL, NULL, 47 },
	...
	{ "cv-logistic-ridge", "( X y units lambdas n-folds -- fr )", "k-fold cross-validation of ridge logistic over a lambdas grid, returning { :lambdas :deviances :best }; X excludes the intercept (added internally, unpenalized), units index rows so per-cluster index arrays give cluster CV", NULL, NULL, NULL, 42 },
	{ "cwd", "( -- path )", "The interpreter's current working directory as a string", "1", "1o", "O(|path|)", 34 },
	{ "data-domain", "( xs ys -- )", "Pin the domain to the padded extents of two data vectors", NULL, NULL, NULL, 47 },
	...
	{ "dgemm-nn", "( α A B β C -- R )", "R = α·A·B + β·C via cblas dgemm — the general form, with the scaling and accumulation the base image's matmul leaves out. Loading this library also gives matmul a size dispatch, so it takes this path above 24×24 of work", NULL, NULL, NULL, 41 },
	{ "dgemm-nt", "( α A B β C -- R )", "R = α·A·Bᵀ + β·C", NULL, NULL, NULL, 41 },
	{ "dgemm-tn", "( α A B β C -- R )", "R = α·Aᵀ·B + β·C", NULL, NULL, NULL, 41 },
	{ "dgemm-tt", "( α A B β C -- R )", "R = α·Aᵀ·Bᵀ + β·C", NULL, NULL, NULL, 41 },
	{ "dgemv-n", "( α A x β y -- r )", "r = α·A·x + β·y via cblas dgemv, with x and y columns (n×1) and a fresh r copied from y; x must be A's column count and y its row count. Reaches BLAS with a vector call rather than dgemm on a one-column matrix", NULL, NULL, NULL, 41 },
	{ "dgemv-t", "( α A x β y -- r )", "r = α·Aᵀ·x + β·y via cblas dgemv, with x A's row count and y its column count", NULL, NULL, NULL, 41 },
	...
	{ "end-process", "( proc -- )", "subprocess.telic: tear down a subprocess frame — close :in/:out/:err and wait on :pid (graceful, blocks until exit)", "3 closes + wait", "none", "O(1)", 36 },
	...
	{ "env", "( name -- val )", "Environment variable as a string, or null if unset (so set-empty \"\" and unset stay distinct)", "1", "1o on hit", "O(|val|)", 34 },
	...
	{ "expand-indicators!", "( design sym -- design )", "Replaces the categorical column sym of a design dataset with one 0/1 indicator column per distinct value above the first (the reference), keyed sym=level, mutating and returning the frame, so dataset>matrix over keys then succeeds; errors when sym is not a column or has fewer than 2 distinct values", NULL, NULL, NULL, 45 },
	...
	{ "ffi-free", "( ptr -- )", "free a C buffer held as a T_PTR (e.g. from malloc) and clear its registry slot. Not for library handles", "free", "none", "O(1)", 38 },
	{ "ffi-function", "( lib symbol arg-types ret-type -- ) <name>", "Resolve symbol in lib, build a libffi call interface, and define the following word <name> to call it. arg-types is an array of type symbols, ret-type a single symbol. The interface is prepared once; calls are ~30–100 ns", "dlsym + prep_cif", "1 binding", "O(argc)", 38 },
	{ "ffi-open", "( path -- lib )", "dlopen the library at path and push a T_PTR handle; \"\" opens the running process itself (dlopen(NULL)) for already-linked symbols. Errors if not found", "dlopen", "1 handle (not GC'd)", "O(1)", 38 },
	{ "ffi-variadic", "( lib symbol arg-types ret-type n-fixed -- ) <name>", "Like ffi-function for a variadic C function: n-fixed leading arguments use the fixed convention, the rest the variadic one (ffi_prep_cif_var). Variadic argument types are fixed per binding, so declare one word per type combination (e.g. a :string setopt and a :long setopt)", "dlsym + prep_cif_var", "1 binding", "O(argc)", 38 },
	{ "figure", "( width height -- )", "Start a new figure of that canvas size, marks empty, styled from the current aes; becomes the current figure", NULL, NULL, NULL, 47 },
	{ "figure!", "( val sym -- )", "Set a property of the current figure — any aes key, or a domain bound (:xmin/:xmax/:ymin/:ymax)", NULL, NULL, NULL, 47 },
	{ "figure>svg", "( -- svg )", "Resolve the domain and render the current figure to an SVG document string", NULL, NULL, NULL, 47 },
	{ "figure@", "( sym -- val )", "Read a property of the current figure", NULL, NULL, NULL, 47 },
	{ "file-exists?", "( path -- bool )", "Whether the path exists; follows symlinks, tests any file type, not just regular files", "1", "none", "O(1)", 34 },
	{ "file-info", "( path -- fr|null )", "io.telic: { :kind :size :modified } for one path — :kind is :file, :directory, or :other, :size the byte count, :modified the modification time as an instant (a quantity in s, whole seconds). Symlinks are followed, so a link to a directory reports :directory; a missing path — a dangling symlink included — answers null rather than erroring, so dup null? if … then guards it", "20", "1o + 1 pair", "O(1)", 34 },
	{ "fill", "( color -- )", "Set the current fill color", NULL, NULL, NULL, 47 },
	...
	{ "find-executable", "( name -- path|null )", "io.telic: the absolute path of name on $PATH (first directory holding it), or null if unset or not found; a name containing / matches no bare PATH entry, so it answers null", "split + probe", "1o per candidate", "O(dirs)", 34 },
	...
	{ "fit-poisson", "( X y max-iterations tolerance -- beta )", "Poisson regression by IRLS, log link", NULL, NULL, NULL, 43 },
	{ "fit-simplex", "( X y -- weights )", "Least squares on the probability simplex — weights ≥ 0 summing to 1 (synthetic-control weights, mixture proportions). The sum constraint enters the nonnegative least-squares solve as a ones row weighted 1000·max(1, max|X|), the weights keep the penalty's O(1/mu²) deviation (~1e-7 relative), and a final renormalization reduces the sum error to one float rounding; throws empty support when every weight is 0", NULL, NULL, NULL, 41 },
	{ "fit-weighted-linear", "( X y w -- beta )", "Weighted least squares: each row of X and y scaled by √wᵢ, then ordinary least squares, so a weight of 2 equals the row appearing twice; unit weights give fit-linear's answer", NULL, NULL, NULL, 41 },
	{ "fit-xgb", "( X y fit-params -- booster )", "Train an XGBoost booster on features X (n×k) and response y (n×1); fit-params are keyed by xgboost parameter name, :rounds drives the boosting loop (default 100). Native-only (libxgboost via FFI)", NULL, NULL, NULL, 44 },
	...
	{ "floats>matrix", "( ptr n -- mat )", "Copy n 32-bit floats from foreign memory at ptr into a fresh n×1 double matrix — the read-back for a C call that returns a float const* result buffer (e.g. predictions); errors if n < 1", "n", "1m(n×1)", "O(n)", 38 },
	...
	{ "glm-covariance", "( X y weights beta family -- cov )", "The p×p coefficient covariance at a fitted beta: φ from glm-dispersion times (XᵀWX)⁻¹, W the IRLS working weight times the prior weights, through one thin SVD of √W·X; standard errors are the square roots of its diagonal. For gaussian-identity with unit weights this is the OLS covariance σ̂²(XᵀX)⁻¹", NULL, NULL, NULL, 43 },
	{ "glm-dispersion", "( X y weights beta family -- phi )", "The dispersion at a fitted beta: 1 when the family's :dispersion is :fixed; otherwise the Pearson estimate Σ wᵢ (yᵢ−μᵢ)² / V(μᵢ) over n−p, p the column count of X. Pass unit weights for an unweighted fit", NULL, NULL, NULL, 43 },
	{ "glm-regression", "( dataset predictors response weights family replications -- model )", "A GLM under family from a dataset: predictors name the design columns (an intercept is prepended); response is the outcome as a column symbol or an n×1 matrix; weights is the prior weights as a column symbol, an n×1 matrix, or null for unit weights. replications 0 gives Wald inference from glm-covariance (so a :pearson family's φ inflates the SEs), B > 0 a nonparametric bootstrap of B refits; the model frame is described above the table", NULL, NULL, NULL, 42 },
	{ "gpd-draw", "( shape scale -- draw )", "one exceedance drawn from the generalized-Pareto tail (shape, scale) by inverse-transform sampling of a uniform [0, 1) variate on the shared RNG stream, so seed fixes the sequence", NULL, NULL, NULL, 46 },
	{ "gpd-fit", "( exceedances -- shape scale )", "maximum-likelihood generalized Pareto (GPD) fit to a vector of threshold exceedances — four refining rounds of a 9×9 grid over (shape, ln scale), reaching about [−0.8, 1.4] in shape at a resolution of 0.02, so a returned shape at either endpoint is the grid boundary, not an optimum", NULL, NULL, NULL, 46 },
	{ "gpd-quantile", "( shape scale p -- q )", "the generalized-Pareto quantile — scale/shape · ((1−p)^−shape − 1), and the exponential limit −scale · ln(1−p) when |shape| < 1e-9; errors unless p is in [0, 1)", NULL, NULL, NULL, 46 },
	...
	{ "intervals", "( lows highs positions -- )", "A horizontal segment from each low to its high at the y position, one 2-point series per interval; equal-length inputs; aes :series-stroke :series-width", NULL, NULL, NULL, 47 },
	...
	{ "linear-regression", "( dataset predictors response replications -- model )", "OLS point estimate; replications 0 gives Wald inference (σ̂²(XᵀX)⁻¹, z and p per coefficient), B > 0 a nonparametric bootstrap over B resamples; the model frame is described above the table", NULL, NULL, NULL, 42 },
	...
	{ "log", "( str level -- )", "io.telic: write one line, the UTC stamp, the level symbol by name and the message, to the log file under TELIC_LOG_DIR/TELIC_LOG_FILE, else to stderr; level is any symbol", "20", "1s per line", "O(|s|)", 35 },
	...
	{ "negative-binomial-log", "( theta -- family )", "The NB2 family at a known dispersion: log link, variance μ + μ²/θ, so a large θ approaches Poisson (negligible overdispersion). Pass to fit-glm to fit coefficients with θ held fixed; theta is curried into the variance quotation, costing one small anonymous word per call", NULL, NULL, NULL, 43 },
	{ "negative-binomial-theta", "( y mu -- theta )", "Maximum-likelihood NB2 dispersion at fitted means, by golden-section search on ln θ over the moment estimate widened a factor of 1000 each way. When the residual variance does not exceed the mean the likelihood rises monotonically in θ and the answer is the top of that window, which reports that Poisson fits rather than a dispersion", NULL, NULL, NULL, 43 },
	...
	{ "panel", "( -- )", "A filled plot ground (:panel-fill) with gridlines as negative space and labeled ticks, no border; call before the data so it draws underneath; on an axis marked categorical it drops the numeric ticks", NULL, NULL, NULL, 47 },
	...
	{ "pcv-logistic-ridge", "( X y units lambdas n-folds -- fr )", "k-fold cross-validation of ridge logistic over a lambdas grid with the (lambda, fold) cells evaluated in parallel, returning { :lambdas :deviances :best }; X excludes the intercept (added internally, unpenalized), units index rows so per-cluster index arrays give cluster CV; the answer matches a serial run, the cells being deterministic", NULL, NULL, NULL, 42 },
	...
	{ "pointer-cell", "( -- ptr )", "Allocate a zeroed pointer-sized cell and return a T_PTR handle to it, for use as a C out-parameter slot (&out) or a one-element handle array; a callee writes a pointer or integer into it, read back with pointer-deref. Freed by ffi-free", "malloc", "1 cell (not GC'd)", "O(1)", 38 },
	{ "pointer-deref", "( ptr -- ptr' )", "Load the pointer stored at cell ptr (*(void**)ptr) and return it as a T_PTR handle — reads a handle a C call wrote into an out-parameter cell, or steps through a T**", "1", "1 handle", "O(1)", 38 },
	...
	{ "run", "( str -- proc )", "subprocess.telic: split a command string on runs of spaces and launch it as a subprocess, returning { :pid :in :out :err }", "split + fork", "1a + 1o frame + 3 streams", "O(|s| + argc)", 36 },
	{ "run-result", "( argv -- frame )", "subprocess.telic: run argv to completion and return { :out :err :status }, closing the streams and reaping the child", "fork + drain", "1fr + output strings", "O(output)", 36 },
	{ "running?", "( pid -- bool )", "Non-blocking liveness via waitid+WNOHANG+WNOWAIT; true while running, false once exited. Non-reaping, so a later wait still returns the status", "1 syscall", "none", "O(1)", 36 },
	...
	{ "save-figure", "( name -- )", "Render the current figure as the next version images-<name>/N.svg (creating the directory); native-only (mkdir via a subprocess)", NULL, NULL, NULL, 47 },
	...
	{ "scatter", "( xs ys -- )", "Scatter markers at the point pairs; aes :point-fill :point-stroke :point-radius; errors on unequal lengths", NULL, NULL, NULL, 47 },
	{ "scatter-plot", "( xs ys -- svg )", "Complete scatter plot in one call — 640×480 figure, axes, scatter, rendered", NULL, NULL, NULL, 47 },
	...
	{ "series", "( xs ys -- )", "Connected line through the points in order, split at NaN gaps; aes :series-stroke :series-width", NULL, NULL, NULL, 47 },
	{ "series-plot", "( xs ys -- svg )", "Complete line plot of ys against xs, rendered", NULL, NULL, NULL, 47 },
	...
	{ "show-figure", "( name -- )", "Open a versioned carousel viewer for images-<name>/ in a browser app window; saves a first version when empty, and later save-figures appear in the carousel; native-only", NULL, NULL, NULL, 47 },
	...
	{ "start-process", "( argv -- proc )", "fork/exec argv[0] with argv as its arguments; return { :pid :in :out :err } (the three streams are T_STREAM)", "fork + 3 pipes", "1o frame + 3 streams", "O(argc)", 36 },
	{ "std", "( mat -- f )", "statistics.telic: the sample standard deviation of all elements", "n", "none", "O(n)", 21 },
	{ "stderr", "( -- stream )", "Standard error as a T_STREAM over fd 2; composes with write/close like any stream", "1", "none", "O(1)", 36 },
	{ "stdin", "( -- stream )", "Standard input as a T_STREAM over fd 0; stdin read reads it whole. (The REPL reads its own program from stdin, so this word suits file-loaded programs.)", "1", "none", "O(1)", 36 },
	{ "stdout", "( -- stream )", "Standard output as a T_STREAM over fd 1; s stdout write emits", "1", "none", "O(1)", 36 },
	{ "stdout>string", "( xt -- str )", "Run xt with descriptor 1 redirected to an unlinked temporary file, restore the descriptor, and answer everything xt wrote — a raw stdout write included, the redirect being at the descriptor rather than in the printing words. Whatever xt leaves on the stack stays, the string on top; stderr is untouched, and a child from start-process writes to its own pipe, not this capture. Captures nest, each call saving its own descriptor. An error or throw out of xt restores the descriptor, discards the captured text, and propagates. Native-only: the wasm build errors, WASI having no temporary files", "2 + xt + bytes", "1o + the temporary file", "O(xt + bytes)", 36 },
	{ "stop", "( pid -- status )", "SIGKILL the child then reap it (137 = 128+9, or its code if it had already exited)", "2 syscalls", "none", "O(1)", 36 },
	...
	{ "stroke", "( color -- )", "Set the current stroke color", NULL, NULL, NULL, 47 },
	...
	{ "svd", "( A -- U S VT )", "Thin singular value decomposition via LAPACKE dgesvd: A = U diag(S) VT, with S the 1×min(m,n) singular values. Column signs of U/VT are not canonical, so pin goldens on S and the reconstruction, not raw U/VT entries", NULL, NULL, NULL, 41 },
	{ "svg-circle", "( cx cy r -- )", "Circle of pixel radius r, current fill and stroke", NULL, NULL, NULL, 47 },
	{ "svg-line", "( x1 y1 x2 y2 -- )", "Line segment in pixel coordinates, current stroke", NULL, NULL, NULL, 47 },
	{ "svg-rect", "( x y w h -- )", "Rectangle at pixel (x, y) of size w×h, current stroke and fill", NULL, NULL, NULL, 47 },
	{ "svg-text", "( x y label -- )", "Text at pixel (x, y), current font size and text-anchor", NULL, NULL, NULL, 47 },
	...
	{ "with-intercept", "( X/design -- X'/design )", "a matrix gets a prepended column of ones, so a fit's beta[0] is the intercept; a design dataset gets an :intercept ones column keyed like any term (errors on an empty design — the rows are read from it)", NULL, NULL, NULL, 45 },
	...
	{ "write", "( str stream -- )", "Write the string's bytes to the stream; loops over partial writes, retries EINTR", "write syscalls", "none", "O(|s|)", 36 },
	{ "write-file", "( str path -- )", "Create or truncate the file, then write the string's bytes", "file write", "none", "O(|s|)", 34 },
	{ "write-in", "( str proc -- )", "subprocess.telic: write the string to the child's :in stream", "write syscalls", "none", "O(|s|)", 36 },
	...
	{ "x-label", "( str -- )", "x-axis title, centered below the tick labels", NULL, NULL, NULL, 47 },
	{ "xgb-free", "( booster -- )", "Free a booster handle", NULL, NULL, NULL, 44 },
	{ "xgb-importance", "( booster importance-type -- scores )", "k×1 per-feature importance, row i is feature i; importance-type is \"gain\", \"weight\", \"cover\", \"total_gain\", or \"total_cover\". Rank with matrix>array argsort reverse", NULL, NULL, NULL, 44 },
	{ "xgb-load", "( path -- booster )", "Read a booster from a saved model file into a fresh handle; free it with xgb-free", NULL, NULL, NULL, 44 },
	{ "xgb-predict", "( booster X -- predictions )", "n×1 scores for an n×k feature matrix; the booster is not freed", NULL, NULL, NULL, 44 },
	{ "xgb-save", "( booster path -- )", "Write the booster in xgboost's own model format; the path extension picks it, .json or .ubj. The model reads back in Python and R", NULL, NULL, NULL, 44 },
