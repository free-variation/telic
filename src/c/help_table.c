...
const HelpEntry help_entries[] = {
	...
	{ "++", "( -- )", "Increment the named local or global variable by 1 in place; only while compiling, a definition or a quotation; a global target is declared ^name in the head; errors on an unknown or non-variable name", "1", "none", "O(1)", 12 },
	...
	{ "--", "( -- )", "Decrement the named local or global variable by 1 in place; only while compiling, a definition or a quotation; a global target is declared ^name in the head; errors on an unknown or non-variable name", "1", "none", "O(1)", 12 },
	...
	{ "1+", "( a -- a+1 )", "float, matrix, exact, or complex", "2 (float)", "matrix 1m(r×c)", "float O(1); matrix O(r×c)", 1 },
	{ "1-", "( a -- a-1 )", "float, matrix, exact, or complex", "2 (float)", "matrix 1m(r×c)", "float O(1); matrix O(r×c)", 1 },
	...
	{ "@i", "( arr/set i -- val )", "Array element; on a set the element at position i in sorted order; on a matrix returns row i as a 1×c matrix", "3 (array)", "matrix 1m(1×c)", "O(1) array and set; O(c) matrix", 16 },
	...
	{ "between?", "( v low high -- bool ) or ( mat/arr low high -- mat )", "core.telic: low ≤ v ≤ high, inclusive; a matrix or array operand answers an element-wise mask, and any ordered scalars compare (strings lexicographic, quantities rescaled within a dimension)", "9", "matrix 3m(r×c)", "O(1); matrix/array O(n)", 4 },
	...
	{ "circle", "( x y r -- )", "The outline of the circle of radius r centred on x y", "n", "none", "O(r)", 34 },
	...
	{ "cls", "( -- )", "Fill the whole bitmap with the current paper", "w·h", "the bitmap, once", "O(w·h)", 34 },
	...
	{ "colors", "( -- frame )", "graphics.telic: the named-color table, symbol to packed 0xRRGGBB, built once from src/c/x11_colors.h — the same table format and print-at read their {name} directives from", "1", "none", "O(1)", 34 },
	...
	{ "ensure", "( body-xt cleanup-xt -- … )", "exceptions.telic: run cleanup-xt (stack-neutral) whether body-xt returns normally or throws/errors, then re-raise on the throw path", "—", "none", "O(body-xt)", 26 },
	...
	{ "expect-throws", "( xt -- )", "test.telic: run xt under catch; pass iff it throws, else throw expected a throw", "—", "none", "O(xt)", 26 },
	...
	{ "first", "( arr/set/matrix -- v )", "core.telic: element 0 of an array, the smallest element of a set, or the first row-major element of a matrix (as a float)", "9", "none", "O(1)", 16 },
	...
	{ "flat-map", "( items xt -- arr )", "arrays.telic: map then flatten, so each element's result array is spliced in, nested arrays inside it included", "n·xt + total", "1a(n) + 1a(total)", "O(n·xt + total)", 25 },
	...
	{ "ink", "( color -- )", "graphics.telic: the color every later shape draws in. Default white", "3", "none", "O(1)", 34 },
	...
	{ "last", "( arr/set/matrix -- v )", "core.telic: the final element of an array, the largest element of a set, or the last row-major element of a matrix (as a float); empty array errors", "9", "none", "O(1)", 16 },
	...
	{ "line", "( x0 y0 x1 y1 -- )", "A one-pixel line between the endpoints, both included", "n", "none", "O(n)", 34 },
	...
	{ "negate", "( a -- -a )", "float, matrix (element-wise), exact, complex, or quantity", "2 (float)", "matrix 1m(r×c)", "float O(1); matrix O(r×c)", 1 },
	...
	{ "reduce", "( arr/set init xt -- val ) or ( dataset init xt -- val )", "Left fold; xt is ( acc elem -- acc ). datasets.telic extends it to a dataset: xt sees each row as a frame keyed by column name, as under map, filter and each, so the fold is ( acc row -- acc )", "3 + n·xt", "none; dataset one row frame each", "O(n·xt)", 25 },
	...
	{ "reify", "( a -- a' )", "Deep copy of any value, dereferencing bound logic vars and recursing into frames, arrays, matrices, strings, sets and a quantity's magnitude (complexes and continuations pass through), but each unbound var becomes a canonical inert symbol :_0, :_1, … numbered by first appearance — a ground, storable, comparable snapshot.", "tree size", "one object per node", "O(tree size)", 17 },
	...
	{ "sq", "( a -- a² )", "float, matrix, exact, or complex", "2 (float)", "matrix 1m(r×c)", "float O(1); matrix O(r×c)", 1 },
	...
	{ "true?", "( a -- bool )", "core.telic: 1 when the value is truthy, else 0. A heap value is truthy by its handle, so an empty string, array, or frame is truthy; a quantity is truthy by its magnitude, a complex when either part is nonzero, an exact when nonzero; 0, null, and an unbound handle are falsy", "3", "none", "O(1)", 4 },
	{ "truncate", "( a -- trunc a )", "trunc", "2", "matrix 1m(r×c)", "float O(1); matrix O(r×c)", 3 },
	{ "try-catch", "( normal-xt err-xt -- … )", "exceptions.telic: run normal-xt; on a throw or interpreter error, run err-xt with the exception (the { :message :trace } error frame, for an interpreter error) on the stack", "—", "1f + 2s on a caught interpreter error", "O(normal-xt)", 26 },
	...
	{ "unit", "( q -- )", "Read the following name; pop a quantity whose magnitude is a positive simple rational, and define a postfix word attaching that unit. The magnitude is the unit's scale relative to its dimension's base (100 cent unit dollar, 1 $ 100 / unit ¢). A single unnamed base dimension gets named after the word", NULL, NULL, NULL, 12 },
	...
	{ "value>bytes", "( v -- str )", "Serialize a value graph to a byte string; nesting deeper than 32768 levels errors rather than overflowing", "n", "1o + buffer growth", "O(n)", 19 },
	...
	{ "with-db", "( path body-xt -- … )", "exceptions.telic: db-open the path, run body-xt ( db -- … ) with the handle, db-close on either exit", "—", "1 db", "O(body-xt)", 26 },
	...
	{ "with-stream", "( stream body-xt -- … )", "exceptions.telic: run body-xt ( stream -- … ) over an already-open stream, close it on either exit", "—", "none", "O(body-xt)", 26 },
...
const HelpExample help_examples[] = {
	...
	{ "++", ": count-up ( -- n ) 0 to tally ++ tally ++ tally tally ; count-up . cr", "2" },
	{ ",", "1 2 , + . cr", "2 3" },
	{ "-", "10 3 - . cr\n[< 1 2 3 >] [< 2 >] - . cr", "7\n[< 1 3 >]" },
	{ "-!", "[ 10 20 ] vector 1 -! matrix>array . cr", "[ 9 19 ]" },
	{ "--", ": count-down ( -- n ) 5 to tally -- tally tally ; count-down . cr", "4" },
	...
	{ "?if", ": keep-if-truthy ( flag -- ) ?if \"kept\" . then . cr ; 5 keep-if-truthy 0 keep-if-truthy", "kept 5\n0" },
	{ "@", "{ :a { :b 5 } } /a/b @ . cr", "5" },
	...
	{ "begin", ": countdown ( n -- ) begin dup . 1- dup 0= until drop cr ; 3 countdown", "3 2 1" },
	...
	{ "colors", "colors :dodgerblue @ . colors size . cr", "2003199 157" },
	...
	{ "dup", "3 dup * . cr", "9" },
	{ "each", "[ 1 2 3 ] ( . ) each cr\n[ [ \"name\" \"age\" ] [ \"ann\" 34 ] [ \"bo\" 25 ] ] true rows>dataset ( :name @ . ) each cr", "1 2 3\nann bo" },
	...
	{ "endcase", ": describe ( v -- ) case :ok of \"fine\" endof \"unmatched: \" . dup . drop \"seen\" endcase . cr ;\n:oops describe", "unmatched:  :oops seen" },
	{ "endof", ": parity ( n -- ) case 0 of \"zero\" endof 1 of \"one\" endof drop \"big\" endcase . cr ;\n1 parity", "one" },
	...
	{ "evaluate", "\": doubled ( n -- n ) 2 * ;\" evaluate\n21 doubled . cr", "42" },
	{ "exact>float", "1/4 exact>float . cr", "0.25" },
	{ "exact?", "1/2 exact? . 0.5 exact? . cr", "1 0" },
	{ "execute", "7 ' 1+ execute . cr", "8" },
	{ "exit", ": early ( n -- ) dup 0 < if drop \"neg\" . cr exit then drop \"pos\" . cr ; -3 early", "neg" },
	{ "exp", "0 exp . cr", "1" },
	...
	{ "f*-", "2 3 10 f*- . cr", "4" },
	{ "f+", "1.5 2.5 f+ . cr", "4" },
	{ "f++", ": fast-up ( -- f ) 1.5 to reading f++ reading reading ; fast-up . cr", "2.5" },
	{ "f-", "5 1.5 f- . cr", "3.5" },
	{ "f--", ": fast-down ( -- f ) 1.5 to reading f-- reading reading ; fast-down . cr", "0.5" },
	{ "f/", "7 2 f/ . cr", "3.5" },
	{ "f1+", "41 f1+ . cr", "42" },
	{ "f1-", "43 f1- . cr", "42" },
	{ "f<", "1 2 f< . cr", "1" },
	...
	{ "first", "[ 7 8 9 ] first . [ 1 2 3 4 ] 2 2 matrix first . [< 9 7 8 >] first . cr", "7 1 7" },
	...
	{ "forget", ": era ( -- n ) 1 ; era . cr forget era : era ( -- n ) 2 ; era . cr", "1\n2" },
	...
	{ "ink", ":dodgerblue ink  0 0 639 479 line", "" },
	{ "inline", ": double ( n -- n ) 2 * ; inline : quadruple ( n -- n ) double double ; 5 quadruple . cr", "20" },
	{ "int-segment", "2 int-segment 7 1 !i dup 0 @i . 1 @i . cr", "0 7" },
	{ "internal", ": helper-word ( -- n ) 3 ; internal : public-word ( -- n ) helper-word 2 * ; public-word . cr", "6" },
	...
	{ "last", "[ 7 8 9 ] last . [ 1 2 3 4 ] 2 2 matrix last . [< 9 7 8 >] last . cr", "9 4 9" },
	{ "leave", ": stop-at-zero ( n -- ) begin dup . 1- dup 0 < if leave then again drop cr ; 2 stop-at-zero", "2 1 0" },
	...
	{ "load", "\": loaded-word ( -- n ) 11 ;\" \"/tmp/docs-load.telic\" write-file \"/tmp/docs-load.telic\" load loaded-word . cr", "11" },
	...
	{ "nhead", "[ [ \"name\" \"age\" ] [ \"ann\" 34 ] [ \"bo\" 25 ] ] true rows>dataset 1 [ :name ] nhead", "name  age\nann    34" },
	{ "nip", "1 2 nip . cr", "2" },
	...
	{ "position-of", "[ 10 20 30 ] 20 position-of . [ 10 20 30 ] 99 position-of . cr\n[< 30 10 20 >] 30 position-of . [ 10 20 30 ] vector 30 position-of . cr", "1 -1\n2 2" },
	...
	{ "print-at", "8 8 \"{red}warm {dodgerblue}cool {plain}plain\" print-at", "" },
	...
	{ "reduce", "[ 1 2 3 4 ] 0 ' + reduce . cr\n[ [ \"name\" \"score\" ] [ \"ann\" 5 ] [ \"bo\" 1 ] ] true rows>dataset 0 ( :score @ + ) reduce . cr", "10\n6" },
	...
	{ "save", ": keep-me ( -- n ) 5 ; \"/tmp/docs-save.telic\" save \"/tmp/docs-save.telic\" read-file \": keep-me\" has? . cr", "1" },
	...
	{ "screen-effect", "0 0 0.6 1 screen-effect", "" },
	{ "screen-frame", ": slide ( -- ) 0 60 1 do k  ( cls k 10 * 100 20 fill-circle ) screen-frame  0.016 sleep  loop ; slide", "" },
	{ "screen-frames", "screen-frames  1 sleep  screen-frames swap - . cr", "" },
	{ "screen-shader", "\"void fxShader(out vec4 color, in vec2 uv) { color = vec4(uv, 0.5, 1.0); }\" screen-shader", "" },
	{ "screen-size", "320 240 screen-size  10 10 plot", "" },
	{ "screen-zoom", "256 192 screen-size  3 screen-zoom  40 30 plot", "" },
	{ "se", "[ 2 4 4 4 5 5 7 9 ] vector se . cr", "0.755929" },
	{ "second", "[ 7 8 9 ] second . cr", "8" },
	{ "see", ": sq-see ( n -- n ) dup * ; ' sq-see see", ": sq-see ( n -- n ) dup * ;" },
	{ "see-compiled", ": sc-demo ( -- f ) 1.5 2.5 f+ ; ' sc-demo see-compiled", ": sc-demo   \\ 5 cells\n 0: (lit) 1.5\n 2: (lf+) 2.5\n 4: exit\n;" },
	{ "see-compiled>string", ": sc-demo2 ( -- f ) 1.5 2.5 f+ ; ' sc-demo2 see-compiled>string print cr", ": sc-demo2   \\ 5 cells\n 0: (lit) 1.5\n 2: (lf+) 2.5\n 4: exit\n;" },
	{ "see-tree", ": st-inner ( -- n ) 1 ; : st-outer ( -- n ) st-inner 2 * ; ' st-outer see-tree", ": st-outer\n  0: st-inner:\n    0: (lit) 1\n    2: exit\n  2: (lit) 2\n  4: *\n  5: exit\n;" },
	...
	{ "vf*", "variable a 3 to a\n: times-a ( f -- v ) vf* a ; 10 times-a . cr", "30" },
	{ "vf+", "variable a 3 to a\n: plus-a ( f -- v ) vf+ a ; 10 plus-a . cr", "13" },
	{ "vf-", "variable a 3 to a\n: minus-a ( f -- v ) vf- a ; 10 minus-a . cr", "7" },
	{ "vf/", "variable a 3 to a\n: over-a ( f -- v ) vf/ a ; 12 over-a . cr", "4" },
	{ "vfabs", "variable a -5 to a\n: abs-a ( -- v ) vfabs a ; abs-a . cr", "5" },
	{ "vfcos", "variable a 0 to a\n: cos-a ( -- v ) vfcos a ; cos-a . cr", "1" },
	{ "vfexp", "variable a 0 to a\n: exp-a ( -- v ) vfexp a ; exp-a . cr", "1" },
	{ "vflog", "variable a 100 to a\n: log-a ( -- v ) vflog a ; log-a . cr", "2" },
	{ "vfneg", "variable a 3 to a\n: neg-a ( -- v ) vfneg a ; neg-a . cr", "-3" },
	{ "vfsin", "variable a 0 to a\n: sin-a ( -- v ) vfsin a ; sin-a . cr", "0" },
	{ "vfsq", "variable a 3 to a\n: sq-a ( -- v ) vfsq a ; sq-a . cr", "9" },
	{ "vfsqrt", "variable a 9 to a\n: root-a ( -- v ) vfsqrt a ; root-a . cr", "3" },
	{ "vftan", "variable a 0 to a\n: tan-a ( -- v ) vftan a ; tan-a . cr", "0" },
	{ "vftanh", "variable a 0 to a\n: tanh-a ( -- v ) vftanh a ; tanh-a . cr", "0" },
	{ "view-figure", "\"plot\" load-library\n\"my-plot\" view-figure", "" },
	{ "vstack", "[ 1 2 ] vector [ 3 4 ] vector vstack matrix>array . cr\n{ :a [ 1 2 ] :b [ :x :y ] } { :a [ 3 ] :b [ :z ] } vstack :a @ transpose matrix>array . cr\nnull [ 5 ] vector vstack matrix>array . cr\n[ 1 2 ] vector m [ 3 4 ] vector m vstack matrix>array . cr", "[ 1 2 3 4 ]\n[ 1 2 3 ]\n[ 5 ]\n[ 1 m 2 m 3 m 4 m ]" },
	{ "vvf*", "variable a 3 to a variable b 4 to b\n: prod-ab ( -- v ) vvf* a b ; prod-ab . cr", "12" },
	{ "vvf*+", "variable b 4 to b variable c 10 to c\n: fma-bc ( f -- v ) vvf*+ b c ; 2 fma-bc . cr", "18" },
	{ "vvf*-", "variable b 4 to b variable c 10 to c\n: fms-bc ( f -- v ) vvf*- b c ; 2 fms-bc . cr", "2" },
	{ "vvf+", "variable a 3 to a variable b 4 to b\n: sum-ab ( -- v ) vvf+ a b ; sum-ab . cr", "7" },
	...
	{ "while", ": halves ( n -- ) begin dup 0 > while dup . 2 quotient repeat drop cr ; 20 halves", "20 10 5 2 1" },
