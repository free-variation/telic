...
- **Dataset verbs** — `select-rows`, `select-columns`, `sort-rows`, `filter`, `map`, `each`, `dim`, `column-type`, and `count` work on a dataset directly, `filter`, `map` and `each` seeing each row as a frame keyed by column name and every column keeping its representation. `column>array` reads any column as an array, `column>set` its distinct values, `column-type` its type (`:numeric` `:datetime` `:quantity` `:text`), and `group-indices` maps each distinct value to its row positions in one sort.
