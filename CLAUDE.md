...
- words.c — tag-dispatched words: arithmetic/comparison via binary_op/
  unary_op, stack ops, printing, format/interpolate and its color
  directives (color_named over x11_colors.h), execute/curry, RNG
...
- arrow.c — Arrow IPC files via vendored nanoarrow (read-arrow/write-arrow):
  the dataset-column to Arrow-type mapping both ways, units carried in
  field metadata.
- platform_posix.c / platform_wasi.c — subprocesses, threads, timing per
  platform (wasi stubs the process words).
- graphics.c — the window: a pixel buffer, `plot`, and the thread-0 event
  pump that owns every Tigr call. It also holds `platform_run_main`, which
  inverts startup so the interpreter runs off thread 0.
...
- docs/reference.md (built-ins and embedded src/forth words) and
  ...
  forth-words.txt, repl_highlight_groups.h, x11_colors.h (tools/gen-colors.py),
