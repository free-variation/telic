...
- exact.c — arbitrary-precision rationals: the limb kernel (add/mul/divmod/
  gcd/decimal/double conversions), the exact algebra behind the polymorphic
  words' T_EXACT branches, the `p/q` and oversized-integer literal parse,
  and float>exact/exact>float/numerator/denominator.
