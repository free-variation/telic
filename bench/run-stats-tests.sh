#!/usr/bin/env bash
#
# Run the heavy statistics benchmarks (telic-only; serial vs parallel bootstrap).
# Ensures the synthetic dataset exists (generated once via gen-logistic-sim.telic),
# then runs each bench/stats/*.telic benchmark with wall-clock time and exit status.
#
...
ensure_sim() {
	...
	( cd "$root" && "$bin" -b < "$here/stats/gen-logistic-sim.telic" ) >&2 || { log "  generation failed"; return 1; }
}
