...
if __name__ == '__main__':
    ...
    for _ in range(loops):
        canvas = render_once(width, height)
    elapsed = pyperf.perf_counter() - t0
    checksum = sum(canvas.bytes)
    print(f"checksum: {checksum}")
    print(f"elapsed: {elapsed:.6f} s ({loops} x {width}x{height})")
