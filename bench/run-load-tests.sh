#!/usr/bin/env bash
#
# Run the telic load / stress tests — the bench/ files that work the
# system hard rather than compare against CPython (run-benchmarks.sh does the
# comparisons). Each is run as `telic -b < file` from the repo root, with
...
run_one() {
	local name=$1
	local file="$here/load/$name.telic"
...
if [ "${#tests[@]}" -eq 0 ]; then
	...
	tests=()
	for t in $(cd "$here/load" && ls *.telic 2>/dev/null | sed 's/\.telic$//'); do
		[ "$t" = select-load ] || tests+=("$t")
	done
	[ -f "$here/load/select-load.telic" ] && tests+=(select-load)
fi
