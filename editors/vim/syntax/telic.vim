...
syn keyword telicBuiltin column>set complete-cases complete-rows complex complex? compose concat conjugate continuation? copy copy-file cor
syn keyword telicBuiltin correlate-with correlation-kendall correlation-pearson correlation-spearman cos cosh count covariance cp cr cross-validate cumulative-sum
...
syn keyword telicBuiltin db-query db? defer delete-at delete-directory delete-file denominator depth diagonal diagonal-matrix difference dim
syn keyword telicBuiltin do dot drop drop-nans dup each ecdf edit edit-distance embodies embodies! emit
syn keyword telicBuiltin end-process endcase endof ensure env env! epoch>date epoch>date-local eq erf erfc evaluate
syn keyword telicBuiltin exact>float exact? exp expand-grid expect expect-near expect-throws expect= f*+ f*- fabs facos
syn keyword telicBuiltin false false? fasin fatan fcos fexp ffi-free ffi-function ffi-open ffi-variadic file-exists? file-info
syn keyword telicBuiltin filter filter-columns find-executable find-first first flat-map flatten fln fln1+ float>exact float? floats>matrix
...
syn keyword telicBuiltin gc gen-each gen-take group-by group-indices halt has? head help histogram-table hstack identity
...
syn keyword telicBuiltin map-frame match match-all matches? matmul matrix matrix-range matrix>array matrix>pointer matrix? max max2
syn keyword telicBuiltin mean median member? merge merge-by mesh min min2 mkdir mod mv n-columns
syn keyword telicBuiltin n-rows name!key name@key nan? ncurry negate neq new-tests nhead nip nlast nmap
syn keyword telicBuiltin nonmissing-count norm not now null? num-cores num-elements numerator of on-tick open-app-window open-file
