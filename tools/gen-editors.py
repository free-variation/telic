...
Emits:
  editors/vim/syntax/telic.vim
  editors/vscode/syntaxes/telic.tmLanguage.json
"""
...
VIM_OUT = os.path.join(ROOT, "editors", "vim", "syntax", "telic.vim")
VSCODE_OUT = os.path.join(ROOT, "editors", "vscode", "syntaxes", "telic.tmLanguage.json")
...
def emit_vim(auto):
    L = []
    L.append('" Vim syntax file for telic (.telic)')
    ...
    L.append('syn match   telicFrameOp  "[@!]\\ze\\k"')
    L.append('syn match   telicFrameKey "[@!]\\@<=\\k\\+"')
    L.append("")
    ...
    L.append(kw("telicKeyword", KEYWORD))
    L.append("")
    L.append(kw("telicLogic", LOGIC))
    L.append(kw("telicBoolean", BOOLEAN))
    L.append("")
    ...
    for chunk in wrap(kw_tokens):
        L.append(kw("telicBuiltin", chunk))
    for tok in match_tokens:
        escaped = "".join("\\" + c if c in "\\.*[]^$~/" else c for c in tok)
        L.append('syn match   telicBuiltin "%s"' % escaped)
    L.append("")
    for grp, link in [
        ...
        L.append("hi def link telic%-12s %s" % (grp, link))
    L.append("")
    L.append('let b:current_syntax = "telic"')
    L.append("")
    return "\n".join(L)
    ...
    grammar = {
        ...
        "repository": {
            ...
            "strings": {
                ...
                "patterns": [
                    {"name": "constant.character.escape.telic", "match": "\"\""},
                    {"name": "constant.other.placeholder.telic", "match": r"\{\d+(?::[^}]*)?\}"},
                ],
            },
            ...
            "constants": {"name": "constant.language.telic", "match": alt(BOOLEAN)},
            "operators": {"name": "keyword.operator.telic", "match": alt(OPERATORS)},
            "builtins": {"name": "support.function.telic", "match": alt(auto)},
            "symbol": {"name": "constant.other.symbol.telic",
                       "match": BOUNDARY_BEHIND + r":[^\s\[\]{};]+"},
            "path": {"name": "constant.other.path.telic",
                     "match": BOUNDARY_BEHIND + r"/[A-Za-z][^\s{};]*"},
            "frameop": {"name": "keyword.operator.frame.telic",
                        "match": r"[@!](?=[A-Za-z])"},
