...
exact "binary_op operands stay rooted" \
  ': pressure ( -- total ) 0 to acc 0 to i begin i 500 < while [ 1 2 3 4 5 6 7 8 ] vector 2 ^ sum acc + to acc f++ i repeat acc ; pressure . cr' \
  "102000 " 500
exact "mod operands stay rooted" \
  ': pressure ( -- total ) 0 to acc 0 to i begin i 500 < while [ 5 7 9 11 ] vector 3 mod sum acc + to acc f++ i repeat acc ; pressure . cr' \
  "2500 " 500
exact "unary_op operand stays rooted" \
  ': pressure ( -- total ) 0 to acc 0 to i begin i 500 < while [ 1 2 3 ] vector exp sum acc + to acc f++ i repeat acc round ; pressure . cr' \
  "15096 " 500
exact "unify rest-pattern operands stay rooted" \
  ': pressure ( -- total ) 0 to acc 0 to i lvar to tail begin i 500 < while [ 1 2 3 ] [ _ tail rest ] ~ drop tail ? 1 @i acc + to acc f++ i repeat acc ; pressure . cr' \
  "1500 " 500
...
exact "@j source matrix stays rooted" \
  ': pressure ( -- total ) 0 to acc 0 to i begin i 500 < while [ 1 2 3 4 ] 2 2 matrix 0 @j sum acc + to acc f++ i repeat acc ; pressure . cr' \
  "2000 " 500
exact "@i row matrix stays rooted" \
  ': pressure ( -- total ) 0 to acc 0 to i begin i 500 < while [ 1 2 3 4 ] 2 2 matrix 0 @i sum acc + to acc f++ i repeat acc ; pressure . cr' \
  "1500 " 500
exact "where source mask stays rooted" \
  ': pressure ( -- total ) 0 to acc 0 to i begin i 500 < while [ 0 1 0 1 1 ] vector where sum acc + to acc f++ i repeat acc ; pressure . cr' \
  "4000 " 500
